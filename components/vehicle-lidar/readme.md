...
An exploratory data analysis application was rapid prototype as a Shinny R application allowing to upload the log data captured and packaged by Unificator server. The application software parses the packages maintains the lidar information extracts the tilt degree and parses the polar coordinate arcs into a data table. The parsed lidar data are being visualised in two scalable vector graphics graphs. The first graph reflects the data as if these were cartesian coordinates and consequently projects them using the related polar to cartesian coordinates transformation. The application can filter the data via a sliding window of 500ms intervals (chunks) as well as maintain the 80th percentile of the data to focus on its closer surrounding data. Find below a snapshot of the analysis application.
